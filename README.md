...
На GitHub вкладка **Contributors** строится по авторам коммитов. Чтобы новые коммиты шли от вас, в репозитории заданы `user.name` / `user.email` под Jwiqen; при клонировании на другой ПК выполните `git config user.name "Jwiqen"` и укажите email, привязанный к вашему GitHub. Файл [.mailmap](.mailmap) помогает выровнять отображение авторов в `git log` для типовых служебных имён; полностью убрать старого участника из истории GitHub можно только переписав историю (например, `git filter-repo`) или оставив репозиторий с новой историей.
